...
export async function scanPathLibrary(rootDir: string, libraryId: number, libraryPathId: number,) {
    ...
    await detectAndRemoveMissingFiles(normalizedRootDir, scannedPaths, libraryId, libraryPathId);
    ...
    while (true) {
        ...
        const jobBatch = files.map((file) => {
            ...
            const relPath = path.relative(normalizedRootDir, normalizedFilePath).replace(/\\/g, '/');
...
async function detectAndRemoveMissingFiles(
    ...
    for (const dbFile of existingFiles) {
        if (dbFile.path.startsWith(rootDir.replace(/\\/g, '/')) && !scannedPaths.has(dbFile.path)) {
    ...
    for (let i = 0; i < missingPaths.length; i += BATCH_DELETE_SIZE) {
        ...
        await db
            .delete(scannedFile)
            .where(inArray(scannedFile.path, batch));
    }
